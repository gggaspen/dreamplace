...
export function useDecorators() {
  ...
  const decorateComponent = useCallback(
    <P>(
  ...
  const applySingleDecorator = useCallback(
    <P>(
      component: ComponentType<P>,
  ...
  const composeDecorators = useCallback(
    <P>(decoratorNames: string[], config?: Record<string, DecoratorConfig>) => {
      if (!registryRef.current) {
        throw new Error('Decorator registry not initialized');
      }
  ...
  const decorateComponents = useCallback(
    <P>(
      components: Array<{
        component: ComponentType<P>;
        decorators?: string[];
  ...
  const canDecorate = useCallback(
    <P>(component: ComponentType<P>, decoratorName: string): boolean => {
  ...
  const createChain = useCallback(() => {
    const chain: {
      ...
      apply<P>(component: ComponentType<P>): ComponentType<P> {
        ...
        const config = this.decorators.reduce(
          (acc, d) => {
            if (d.config) {
              acc[d.name] = d.config;
            }
    ...
    monitoring: useCallback(
      <P>(component: ComponentType<P>) => {
        return decorateComponent(component, ['errorBoundary', 'performance', 'analytics'], {
          ...
          performance: {
            enabled: true,
            options: { measureRender: true, measureInteraction: true },
          },
