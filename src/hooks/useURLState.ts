...
import { URLStateConfig, URLStateConfigs, urlStateManager } from '@/infrastructure/routing/URLStateManager';
 ...
 */
export function useURLStateMultiple<T extends Record<string, any>>(
 ...
 */
export function useURLString(paramName: string, defaultValue: string = ''): [string, (value: string) => void] {
  return useURLState(URLStateConfigs.string(paramName, defaultValue));
}

export function useURLNumber(paramName: string, defaultValue: number = 0): [number, (value: number) => void] {
  return useURLState(URLStateConfigs.number(paramName, defaultValue));
}

export function useURLBoolean(paramName: string, defaultValue: boolean = false): [boolean, (value: boolean) => void] {
...
export function useURLArray<T>(
  ...
  return useURLState(URLStateConfigs.array(paramName, defaultValue, itemSerializer, itemDeserializer));
}

export function useURLDate(paramName: string, defaultValue: Date = new Date()): [Date, (value: Date) => void] {
...
export function useURLPagination(defaults: PaginationState = { page: 1, limit: 10 }): [
  PaginationState,
  ...
  const updatePagination = useCallback((updates: Partial<PaginationState>) => {
    ...
    if (updates.sortBy !== undefined || updates.sortOrder !== undefined || updates.limit !== undefined) {
      updates.page = 1;
    }
    setState(updates);
  }, [setState]);
...
export function useURLFilters<T extends Record<string, any>>(
  filterConfigs: { [K in keyof T]: URLStateConfig<T[K]> }
