      ...
      <>
        ...
        >
          <VStack spacing={1} align={orientation === 'vertical' ? 'center' : 'start'}>
            <Heading size={size === 'lg' ? 'lg' : 'md'} lineHeight='shorter'>
              {artist.name}
            </Heading>
          ...
          {artist.bio && orientation === 'vertical' && (
            <Body size='sm' variant='muted' noOfLines={2} textAlign='center'>
              {artist.bio}
            </Body>
          )}
          ...
          {showSocialLinks && socialLinksArray.length > 0 && (
            <HStack spacing={2} pt={1}>
              {socialLinksArray.map(([platform, url]) => {
                ...
                return (
                  <Link key={platform} href={url} isExternal onClick={e => e.stopPropagation()}>
                    <IconButton
