...
export class DataCommand<T = unknown> extends BaseCommand<T> {
  ...
  constructor(payload: DataPayload<T>, dataService: any, context?: Record<string, unknown>) {
    super(DataCommand.getCommandType(payload.operation), payload, context);
  ...
  private static getCommandType(operation: string): CommandType {
    switch (operation) {
      case 'create':
        return CommandType.CREATE_ITEM;
      case 'update':
        return CommandType.UPDATE_ITEM;
      case 'delete':
        return CommandType.DELETE_ITEM;
      case 'fetch':
        return CommandType.FETCH_DATA;
      default:
        return CommandType.FETCH_DATA;
    }
  }
